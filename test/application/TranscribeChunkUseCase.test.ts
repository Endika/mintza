...
const meeting = (): Meeting => Meeting.start({ template: Template.work(), language: Language.of('en') });
const chunk = (peakLevel: number): AudioChunk =>
  new AudioChunk({ blob: new Blob(['x'], { type: 'audio/webm' }), startMs: 0, endMs: 15000, mimeType: 'audio/webm', peakLevel });
...
describe('TranscribeChunkUseCase silence skipping', () => {
  it('skips a near-silent chunk without calling transcription', async () => {
    ...
    const m = meeting();
    const result = await new TranscribeChunkUseCase(port).execute({ meeting: m, chunk: chunk(0.001) });
